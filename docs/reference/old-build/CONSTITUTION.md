> **Editor's note (22 Sep 2026):** this is the previous build's `CLAUDE.md` (`D:\projects\Sentinel_Repo\CLAUDE.md`, 13 Sep 2026), archived **verbatim** below and renamed so that Claude Code never loads it as instructions — it dates itself to 13 September and forbids authentication beyond a demo role, both false for the fresh build. **Nothing in it applies; the root `CLAUDE.md` governs.** Its rule sets 2–4 and 6 are carried over verbatim into the root `CLAUDE.md` where they still hold.
