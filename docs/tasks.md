...
| S3.0 | **Login, roles, sessions and public-exposure hardening (schema v2)** — new in v2.3 | Wed 23 | 2 h | |
| S3.7 | **Catalogue adapter (`/api/ingest` + `cameras.json`) and the demo-feed onboarding runbook** — new in v2.3 | Wed 23 | 45 min | |
...
| S3.2 | Frontend foundation: scaffold, data layer, **login page** (v2.3), IST time, Map, Cameras, Watchlist | Wed 23 | 2.5 h | |
...
| S3.5 | **Hosting go-live: tunnel, evaluator accounts, 24/7 runbook** — new in v2.3 | Thu 24 | 1.5 h | |
| S3.6 | **[Adi + Claude] Own-footage ground truth: a real hit and a real multi-camera route** — new in v2.3 | Thu 24 | 2 h | |
...
Budgets total ≈ 53 h of sessions over Mon–Sat (v2.3 added about 6 h: S3.0, S3.5, S3.6, S3.7, S6.1b). They are session budgets, not promises: when a session overruns, the cut order below decides what goes, never the documents.
...
### S0.3 — [Adi] Confirm the entry category `[ ]`
...
### S3.0 — Login, roles, sessions and public-exposure hardening (schema v2) `[ ]`
...
### S3.7 — Catalogue adapter (`/api/ingest` and `cameras.json`) and the demo-feed runbook `[ ]`
*New in v2.3, decision F45.*
...
### S3.2 — Frontend foundation: scaffold, data layer, login page, IST time, Map, Cameras, Watchlist `[ ]`
...
*Build:*
- **[Adi] films:** one vehicle with a clearly readable plate, passing **two or three real locations** a few minutes apart (a gate, a junction, a lane entrance — anywhere the plate is legible and roughly 40 px wide or more). Landscape, steady, 30–60 s each. Note the real coordinates and a plausible department for each location.
- `scripts/replay_publish.py --many` publishes each clip as its own RTSP path (`local01`, `local02`, `local03`) through mediamtx, all bound to `127.0.0.1`.
...
*Build:* **rewrite the top of `README.md` for a judge** (what it is, what it needs, `python launch.py start`, signing in, the demo walkthrough, the disclosed limitations — seeded geography, the labelled demo vehicle, the tiered active set — and a short troubleshooting list), moving the session instructions under `docs/`; **verify the hosted URL and the evaluator credentials from a private window on another network** and record both in the submission form (never in the repo); then tick every box against an observation; sweep for credentials across the history, the deliverables and `data/screens` (`git log -p | grep -E "(rtsp|https?)://[^<{/ ]+:[^<{/ *]+@"` must print only the S1.1 example if anything); every document exported to PDF; numbers labelled; the 30-vs-50 note present; `frontend/dist` zipped to `deliverables/frontend-dist.zip` (decision F30 — `dist/` itself stays ignored); `git tag -a v2.0-submission` on `main`.
