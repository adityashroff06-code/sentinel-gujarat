...
**`camera_id` hygiene (B11):** `camera_id` matches `^[A-Za-z0-9_-]{1,64}$` — it flows into filesystem paths and URLs, so nothing else is accepted, at every boundary. `rtsp_url_template` may carry `<email>`/`<password>` placeholders (filled from the environment in memory, **and only when the template's host and port are the configured sandbox gateway** — `config.is_sandbox_gateway()`, shared by the worker and the health probe; any other host is used as-is, placeholders and all, so a registered camera can never steer the organisers' credentials elsewhere — 25 Sep review gate) **or** be a plain local URL used as-is; no stored URL ever contains a credential.
...
| GET | `/cameras/activity` | `?hours=` (1–168, default 24) — the GIS Activity layer: `[{camera_id, sightings, plates, alerts, last_seen, by_provenance{live\|harvest\|demo\|test: count}, alerts_by_provenance{live\|harvest\|demo\|test: count}}]`, one row per camera with any read or alert whose `seen_at` / `fired_at` is at or after the server's wall clock minus `hours`; idle cameras are omitted; `plates` counts distinct plates; `by_provenance` splits `sightings` and `alerts_by_provenance` splits `alerts` so a demo read or a demo alert never passes as a live one (root rule 12) — an alert's provenance is its sighting's (watchlist) or event's (zone), else its `clock_source` mapped as F26 maps it (`demo` → demo, unknown → test, never live); `alerts` stays the total; sorted by reads, then alerts — registered **before** `/{camera_id}` |
...
| POST | `/cameras` | manual onboarding (Model 1 deliverable). Evaluator or admin; an evaluator creates a registered, non-catalogue row — `fps_tier` other than `registered` or `source='catalogue'` is **403** for anyone but an admin (the tier stays admin-only, as on PATCH) |
...
| GET | `/plates/suggest` | `?limit=` (1–50, default 8) → `{demo, watchlist, top_live}`, items `{plate, reads, cameras, last_seen, provenance, on_watchlist, camera_ids}` — the "plates to try" for Search: `demo` = the labelled demo plates (watchlisted first, then reads); `watchlist` = active unexpired entries (seen first, reads over every provenance joined on the canonical fold, `provenance` of the latest read, null if never seen; then severity); `top_live` = the most-read structurally full `live` plates, grouped by their coerced form. `cameras` is a count; `camera_ids` (sorted) is filled for `top_live` only, so a caller can name where a live read came from (the Command panel prefers a plate read on an organisers' camera over one read on a stock-footage feed). Audited like every `/plates/*` query (B12) |
