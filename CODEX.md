...
- **Login and public exposure (added 22 Sep — the platform is published to the internet for the judges; decisions F41, F42, `docs/api.md` §9):** password storage (scrypt, per-user salt, never a plain hash, never in `.env` or a log); session cookies (`HttpOnly`, `SameSite=Strict`, `Secure` when public, server-side revocation, real expiry); the login throttle surviving a restart; `Origin` checked on mutations; role ladder enforced on the server, not only hidden in the UI; every route except the login, health and static assets refusing an unauthenticated request — including any mount, which bypasses app-level dependencies; security headers present; rate limits on route, reports and the relay; only the API port published, with mediamtx and Vite on `127.0.0.1`. Check that an audit row names the user and never carries a password or an attempted value.
- **Demonstration honesty (F43, F46):** a real read and an injected one must be distinguishable in the UI, the exports and the videos; check that provenance survives every path a judge can see, and that nothing labelled `live` came from a seeder.
