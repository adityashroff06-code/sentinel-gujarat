...
- **System B: a local camera server.** A mediamtx server on the laptop publishes 28 feeds (`local01` to `local28`) from stock traffic footage: sample clips, not sandbox footage and not filmed by the team. Each camera record says so in its location name and carries seeded coordinates. Four carry the active tier and have been analysed through the same live path in verification runs; they join the analysed set whenever the node's camera cap allows. The other 24 are view-only.
